import styled , {css} from 'styled-components';
import {ArrowLeft,Home,Search,Notifications,Email} from "../../styles/Icons";

export const Container = styled.div`
  display:flex;
  flex-direction:column;

  width: min(601px , 100%); // é o tamanho máximo
  
  @media(min-width:500px){
    border-left:1px solid var(--outline);
    border-right:1px solid var(--outline);
  }
`;

export const  Header = styled.header`
  z-index:2; // para ficar acima de tudo 
  display:flex;
  align-items:center;
  text-align:left;

  position:sticky;
  top:0;
  background: var(--primary);

  padding: 8px 0 9px 13px;
  border-bottom:1px solid var(--outline);

  >button{
    padding:8px;
    border-radius:50px;

    outline:0;
    cursor:pointer;

    &:hover{
      background: var(--twitter-dark-hover);
    }
  }
`;
export const BackIcon = styled(ArrowLeft)`
  width: 24px;
  height:24px;
  fill: var(--twitter)
`;

export const ProfileInfo = styled.div`
  margin-left:17px;
  display:flex;
  flex-direction:column;

  >strong{
    font-size:19px
  }
  >span{
    font-size:17px;
    color: var(--gray);
  }

`;

  /* export const ProfilePage = styled.div``; */
  export const BottomMenu = styled.div`
    position:fixed;
    bottom:0;
    left:0;
    z-index:2;

    background: var(--primary);
    width:100%;
    border-top:solid 1px var(--outline);

    display:flex;
    align-items:center;
    justify-content:space-between;
    padding: 8px min(46px, max(10vw,10px)); //barra troca de tamanho de acordo com o tamanho da tela

    @media(min-width:500px){
      display:none;
    }
 
  `;

  const IconCSS = css`
    width:31px;
    height:31px;

    cursor:pointer;
    fill: var(--gray);
    &:hover, &.active{
      fill: var(--twitter)
    }
    
  `;

  export const HomeIcon = styled(Home)`${IconCSS}`;
  export const SearchIcon = styled(Search)`${IconCSS}`;
  export const BellIcon = styled(Notifications)`${IconCSS}`;
  export const EmailIcon = styled(Email)`${IconCSS}`;

  